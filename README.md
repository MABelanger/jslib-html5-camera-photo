...
the cameraPhoto.startCamera(cameraDevice, resolution)
...
cameraDevice (facingMode or deviceId)
...
#### Start with ideal (cameraDevice & resolution)

```js
// example of ideal resolution 640 x 480
cameraPhoto
  .startCamera(cameraDevice, { width: 640, height: 480 })
  .then((stream) => {
    /* ... */
  })
  .catch((error) => {
...
it will try the range of width `[3840, 2560, 1920, 1280, 1080, 1024, 900, 800, 640, default]` px to take the maximum width of `3840`px if it can't, `2560`px and so on ... until the fall back of the default value of the camera. The cameraDevice is optional.

```js
// It will try the best to get the maximum resolution with the specified cameraDevice
cameraPhoto
  .startCameraMaxResolution(cameraDevice)
  .then((stream) => {
    /* ... */
  })
  .catch((error) => {
